...
function update(W,H){
 ...
 const ox=p.x,oy=p.y;
p.x=clamp(p.x+Math.cos(p.a)*p.s,20,MOTIWorld.world.w-20);
p.y=clamp(p.y+Math.sin(p.a)*p.s,20,MOTIWorld.world.h-20);
if(MOTIWorld.hitsBuilding(p.x,p.y,16)){
  p.x=ox;p.y=oy;p.s*= -0.18;
  if(Math.abs(p.s)>.4) MOTIGame.toast("🏠 ¡Cuidado con el edificio!");
}
if(p.inv>0)p.inv--;
