...
async function cargarLocalidades() {
    if (!listaLocalidades || cargandoLocalidades || localidadesCargadas) return;

    cargandoLocalidades = true;
    console.log("📍 MOTI GO: cargando localidades directamente desde Firestore → destinos...");

    try {
        const snapshot = await getDocs(collection(db, "destinos"));

        console.log("📍 MOTI GO: documentos encontrados en destinos:", snapshot.size);
        ...
        snapshot.forEach((docSnap) => {
            ...
            // La colección destinos es la fuente oficial de localidades.
            // Si el documento trae municipio, comprobamos que sea Ostuacán.
            // Si no trae municipio, no lo descartamos automáticamente.
            const municipioDocumento = normalizarTexto(datos.municipio);
            const municipioCorrecto =
                !municipioDocumento ||
                municipioDocumento === normalizarTexto(MUNICIPIO_MOTIGO);

            if (datos.activo === false || !municipioCorrecto) {
                return;
            }
            ...
            localidadesDisponibles.push({
                ...
                municipio: datos.municipio || MUNICIPIO_MOTIGO
            });
        });

        // Evitamos duplicados por nombre aunque existan documentos repetidos.
        const mapaLocalidades = new Map();
        localidadesDisponibles.forEach((localidad) => {
            const clave = normalizarTexto(localidad.nombre);
            if (!mapaLocalidades.has(clave)) {
                mapaLocalidades.set(clave, localidad);
            }
        });
        localidadesDisponibles = Array.from(mapaLocalidades.values());
        ...
        console.log(
            "📍 MOTI GO: localidades disponibles para el registro:",
            localidadesDisponibles.length,
            localidadesDisponibles.map((l) => l.nombre)
        );

        if (localidadInput && document.activeElement === localidadInput) {
            mostrarSugerenciasLocalidad();
        }
    } catch (error) {
        console.error("❌ MOTI GO: error cargando localidades desde destinos:", error);
        localidadesDisponibles = [];
        localidadesCargadas = false;
        listaLocalidades.innerHTML = "";

        if (sugerenciasLocalidad) {
            sugerenciasLocalidad.innerHTML =
                '<div class="localidad-sin-resultados">No pudimos cargar las localidades. Intenta nuevamente.</div>';
...
function mostrarSugerenciasLocalidad() {
    ...
    if (cargandoLocalidades || !localidadesCargadas) {
        sugerenciasLocalidad.innerHTML =
            '<div class="localidad-sin-resultados">Cargando localidades...</div>';
        sugerenciasLocalidad.classList.add("visible");
...
if (localidadInput) {
    ...
    localidadInput.addEventListener("focus", () => {
        mostrarSugerenciasLocalidad();
        if (!localidadesCargadas && !cargandoLocalidades) {
            cargarLocalidades();
        }
    });
