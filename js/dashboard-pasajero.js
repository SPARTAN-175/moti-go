    ...
    listenerPedidoActivoCliente =
        onSnapshot(
            ...
            snapshot => {
                ...
                // =================================================
                // CALIFICACIÓN DESPUÉS DE LA ENTREGA
                // =================================================
                if (
                    pedidoActual.estado === "entregado" &&
                    pedidoActual.entregaConfirmada === true &&
                    !pedidoActual.calificacionRepartidor?.estrellas
                ) {

                    mostrarCalificacionRepartidor(
                        pedidoActual
                    );
...
// =====================================================
// MOTI GO - CALIFICAR REPARTIDOR
// =====================================================

function mostrarCalificacionRepartidor(pedido) {

    if (!pedido?.repartidorId) return;

    if (pedido.calificacionRepartidor?.estrellas) return;

    if (document.getElementById("motiGoCalificacionModal")) return;

    const modal = document.createElement("div");
    modal.id = "motiGoCalificacionModal";

    modal.innerHTML = `
        <div class="moti-go-rating-overlay">
            <div class="moti-go-rating-card">
                <div class="moti-go-rating-icon">⭐</div>
                <h2>¿Cómo fue tu entrega?</h2>
                <p>Califica al repartidor para ayudarnos a mejorar el servicio de MOTI GO.</p>
                <div class="moti-go-rating-stars" role="radiogroup" aria-label="Calificación de la entrega">
                    ${[1,2,3,4,5].map(n => `
                        <button type="button" class="moti-go-rating-star" data-rating="${n}" aria-label="${n} ${n === 1 ? "estrella" : "estrellas"}">☆</button>
                    `).join("")}
                </div>
                <div id="motiGoRatingTexto" class="moti-go-rating-texto">Selecciona una calificación</div>
                <button type="button" id="motiGoRatingGuardar" class="moti-go-rating-guardar" disabled>Enviar calificación</button>
                <button type="button" id="motiGoRatingOmitir" class="moti-go-rating-omitir">Ahora no</button>
            </div>
        </div>
    `;
    ...
    let seleccion = 0;
    const estrellas = modal.querySelectorAll(".moti-go-rating-star");
    const texto = modal.querySelector("#motiGoRatingTexto");
    const guardar = modal.querySelector("#motiGoRatingGuardar");
    const textos = { 1:"Muy mala", 2:"Mala", 3:"Regular", 4:"Buena", 5:"Excelente" };

    estrellas.forEach(boton => {
        boton.addEventListener("click", () => {
            seleccion = Number(boton.dataset.rating);
            estrellas.forEach(item => {
                item.textContent = Number(item.dataset.rating) <= seleccion ? "★" : "☆";
                item.classList.toggle("seleccionada", Number(item.dataset.rating) <= seleccion);
            });
            texto.textContent = textos[seleccion];
    ...
    modal.querySelector("#motiGoRatingOmitir")?.addEventListener("click", () => modal.remove());
...
function mostrarTicketPedido(
    ...
    // El esquema actual del pedido guarda los totales directamente.
    // Los campos antiguos de pago no existen y provocaban $0.00.
    const subtotal =
        Number(
            pedido.subtotal ??
            0
        ...
        ?.addEventListener(
            ...
            async () => {
                ...
                try {
                    ...
                    console.error(
                        "❌ MOTI GO: no se pudo generar el ticket como imagen:",
                        error
                    );
...
// =====================================================
// MOTI GO - CARGAR HTML2CANVAS BAJO DEMANDA
// =====================================================

let motiGoHtml2CanvasPromise = null;

function cargarHtml2CanvasMotiGo() {

    if (window.html2canvas) return Promise.resolve(window.html2canvas);
    if (motiGoHtml2CanvasPromise) return motiGoHtml2CanvasPromise;

    motiGoHtml2CanvasPromise = new Promise((resolve, reject) => {

        const existente = document.querySelector('script[data-motigo-html2canvas="true"]');

        if (existente) {
            existente.addEventListener("load", () => resolve(window.html2canvas));
        ...
        script.async = true;
        script.dataset.motigoHtml2canvas = "true";
    ...
    return motiGoHtml2CanvasPromise;
...
// =====================================================
// MOTI GO - ESTILOS DE CALIFICACIÓN
// =====================================================

if (!document.getElementById("motiGoRatingStyles")) {
    const estilosRating = document.createElement("style");
    estilosRating.id = "motiGoRatingStyles";
    estilosRating.textContent = `
        #motiGoCalificacionModal { position:fixed; inset:0; z-index:1000000; font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
        .moti-go-rating-overlay { position:fixed; inset:0; background:rgba(15,23,42,.72); display:flex; align-items:center; justify-content:center; padding:20px; box-sizing:border-box; }
        .moti-go-rating-card { width:min(420px,100%); background:#fff; border-radius:26px; padding:30px 24px 22px; text-align:center; box-shadow:0 25px 70px rgba(0,0,0,.28); }
        .moti-go-rating-icon { width:64px; height:64px; margin:0 auto 16px; border-radius:50%; background:#fff7ed; display:flex; align-items:center; justify-content:center; font-size:32px; }
        .moti-go-rating-card h2 { margin:0 0 8px; color:#111827; font-size:23px; }
        .moti-go-rating-card p { margin:0 auto 20px; max-width:330px; color:#64748b; font-size:14px; line-height:1.5; }
        .moti-go-rating-stars { display:flex; justify-content:center; gap:4px; margin:8px 0; }
        .moti-go-rating-star { border:0; background:transparent; color:#cbd5e1; font-size:42px; line-height:1; padding:4px; cursor:pointer; transition:transform .15s ease,color .15s ease; }
        .moti-go-rating-star:hover,.moti-go-rating-star.seleccionada { color:#f59e0b; transform:translateY(-2px); }
        .moti-go-rating-texto { min-height:20px; margin:4px 0 18px; color:#475569; font-size:13px; font-weight:700; }
        .moti-go-rating-guardar { width:100%; border:0; border-radius:14px; padding:14px 16px; background:#111827; color:#fff; font-size:15px; font-weight:700; cursor:pointer; }
        .moti-go-rating-guardar:disabled { opacity:.45; cursor:not-allowed; }
        .moti-go-rating-omitir { width:100%; border:0; background:transparent; padding:11px; margin-top:5px; color:#64748b; font-size:13px; cursor:pointer; }
    `;
    document.head.appendChild(estilosRating);
}
